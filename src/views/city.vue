<template>
	<div class="citylist">
		<div id="crm">
			<p>热门城市</p>
			<div class="chot">
				<p v-for="ho in hotcity" @click="handlecity(ho.id,ho.nm)">{{ho.nm}}</p>
			</div>
		</div>
		
		<div class="en" v-for="i in en">
			<p :id="i" style="height: 1px;"></p>
			<p>{{handleen(i)}}</p>
			<div class="zh" v-for="ci in city.cts" v-if="handlezh(ci.py,i)">
		...
		<div class="ciright" @touchstart.stop.prevent=""  @touchmove.stop.prevent="">
			...
			<p v-for="it in en" @touchend="scrollTo(it)">{{handleen(it)}}</p>
		</div>
	</div>
</template>

<script>
import citylist from '../../static/citys.json'
export default {
  name: 'city',
  created(){
//	console.log(citylist.cts)
	...
	this.en = this.en.sort();
  	...
  	this.$store.commit('changeHT','选择城市');
  },
  computed:{
  ...
  methods:{
		...
		handlecity(id,nm){
			this.$store.commit('changeCI',{'id':id,'nm':nm});
